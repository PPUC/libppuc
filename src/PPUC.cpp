...
void
PPUC::SetLogMessageCallback(PPUC_LogMessageCallback callback, const void* userData) {
...
void
PPUC::LoadConfiguration(const char* configFile) {
...
void
PPUC::SetSerial(const char* serial) {
...
void
PPUC::SendLedConfigBlock(const YAML::Node& items, uint32_t type, uint8_t board, uint32_t port) {
    for (YAML::Node n_item : items) {
        ...
        m_pRS485Comm->SendConfigEvent(
            new ConfigEvent(board, (uint8_t)CONFIG_TOPIC_LAMPS, index++, (uint8_t)CONFIG_TOPIC_PORT, port));
        m_pRS485Comm->SendConfigEvent(
            new ConfigEvent(board, (uint8_t)CONFIG_TOPIC_LAMPS, index++, (uint8_t)CONFIG_TOPIC_TYPE, type));
        m_pRS485Comm->SendConfigEvent(new ConfigEvent(board, (uint8_t)CONFIG_TOPIC_LAMPS, index++,
        ...
        ss >> color;
        m_pRS485Comm->SendConfigEvent(
            new ConfigEvent(board, (uint8_t)CONFIG_TOPIC_LAMPS, index++, (uint8_t)CONFIG_TOPIC_COLOR, color));
    }
}
...
PPUC::Connect() {
    if (m_pRS485Comm->Connect(m_serial)) {
        ...
        for (YAML::Node n_board : boards) {
            m_pRS485Comm->SendConfigEvent(new ConfigEvent(n_board["number"].as<uint8_t>(),
                                                          (uint8_t)CONFIG_TOPIC_PLATFORM, 0,
        ...
        if (switches) {
            for (YAML::Node n_switch : switches) {
                ...
                index = 0;
                m_pRS485Comm->SendConfigEvent(
                    new ConfigEvent(n_switch["board"].as<uint8_t>(), (uint8_t)CONFIG_TOPIC_SWITCHES, index++,
        ...
        if (switchMatrix) {
            index = 0;
            m_pRS485Comm->SendConfigEvent(
                new ConfigEvent(switchMatrix["board"].as<uint8_t>(), (uint8_t)CONFIG_TOPIC_SWITCH_MATRIX, index++,
                                (uint8_t)CONFIG_TOPIC_ACTIVE_LOW, switchMatrix["activeLow"].as<bool>()));
            m_pRS485Comm->SendConfigEvent(
                new ConfigEvent(switchMatrix["board"].as<uint8_t>(), (uint8_t)CONFIG_TOPIC_SWITCH_MATRIX, index++,
                                (uint8_t)CONFIG_TOPIC_MAX_PULSE_TIME, switchMatrix["pulseTime"].as<uint32_t>()));
            ...
            for (YAML::Node n_switchMatrixColumn : switcheMatrixColumns) {
                m_pRS485Comm->SendConfigEvent(new ConfigEvent(switchMatrix["board"].as<uint8_t>(),
                                                              (uint8_t)CONFIG_TOPIC_SWITCH_MATRIX, index++,
                                                              (uint8_t)CONFIG_TOPIC_TYPE, MATRIX_TYPE_COLUMN));
                m_pRS485Comm->SendConfigEvent(
                    new ConfigEvent(switchMatrix["board"].as<uint8_t>(), (uint8_t)CONFIG_TOPIC_SWITCH_MATRIX, index++,
                                    (uint8_t)CONFIG_TOPIC_NUMBER, n_switchMatrixColumn["number"].as<uint32_t>()));
                m_pRS485Comm->SendConfigEvent(
                    new ConfigEvent(switchMatrix["board"].as<uint8_t>(), (uint8_t)CONFIG_TOPIC_SWITCH_MATRIX, index++,
                                    (uint8_t)CONFIG_TOPIC_PORT, n_switchMatrixColumn["port"].as<uint32_t>()));
            }
            ...
            for (YAML::Node n_switchMatrixRow : switcheMatrixRows) {
                ...
                m_pRS485Comm->SendConfigEvent(
                    new ConfigEvent(switchMatrix["board"].as<uint8_t>(), (uint8_t)CONFIG_TOPIC_SWITCH_MATRIX, index++,
                                    (uint8_t)CONFIG_TOPIC_NUMBER, n_switchMatrixRow["number"].as<uint32_t>()));
                m_pRS485Comm->SendConfigEvent(
                    new ConfigEvent(switchMatrix["board"].as<uint8_t>(), (uint8_t)CONFIG_TOPIC_SWITCH_MATRIX, index++,
                                    (uint8_t)CONFIG_TOPIC_PORT, n_switchMatrixRow["port"].as<uint32_t>()));
            }
        }
        ...
        if (pwmOutput) {
            for (YAML::Node n_pwmOutput : pwmOutput) {
                ...
                index = 0;
                m_pRS485Comm->SendConfigEvent(
                    new ConfigEvent(n_pwmOutput["board"].as<uint8_t>(), (uint8_t)CONFIG_TOPIC_PWM, index++,
                                    (uint8_t)CONFIG_TOPIC_PORT, n_pwmOutput["port"].as<uint32_t>()));
                m_pRS485Comm->SendConfigEvent(
                    new ConfigEvent(n_pwmOutput["board"].as<uint8_t>(), (uint8_t)CONFIG_TOPIC_PWM, index++,
                                    (uint8_t)CONFIG_TOPIC_NUMBER, n_pwmOutput["number"].as<uint32_t>()));
                m_pRS485Comm->SendConfigEvent(
                    new ConfigEvent(n_pwmOutput["board"].as<uint8_t>(), (uint8_t)CONFIG_TOPIC_PWM, index++,
                                    (uint8_t)CONFIG_TOPIC_POWER, n_pwmOutput["power"].as<uint32_t>()));
                m_pRS485Comm->SendConfigEvent(
                    new ConfigEvent(n_pwmOutput["board"].as<uint8_t>(), (uint8_t)CONFIG_TOPIC_PWM, index++,
                                    (uint8_t)CONFIG_TOPIC_MIN_PULSE_TIME, n_pwmOutput["minPulseTime"].as<uint32_t>()));
                m_pRS485Comm->SendConfigEvent(
                    new ConfigEvent(n_pwmOutput["board"].as<uint8_t>(), (uint8_t)CONFIG_TOPIC_PWM, index++,
                                    (uint8_t)CONFIG_TOPIC_MAX_PULSE_TIME, n_pwmOutput["maxPulseTime"].as<uint32_t>()));
                m_pRS485Comm->SendConfigEvent(
                    new ConfigEvent(n_pwmOutput["board"].as<uint8_t>(), (uint8_t)CONFIG_TOPIC_PWM, index++,
                                    (uint8_t)CONFIG_TOPIC_HOLD_POWER, n_pwmOutput["holdPower"].as<uint32_t>()));
                m_pRS485Comm->SendConfigEvent(new ConfigEvent(n_pwmOutput["board"].as<uint8_t>(),
                                                              (uint8_t)CONFIG_TOPIC_PWM, index++,
                                                              (uint8_t)CONFIG_TOPIC_HOLD_POWER_ACTIVATION_TIME,
                                                              n_pwmOutput["holdPowerActivationTime"].as<uint32_t>()));
                m_pRS485Comm->SendConfigEvent(
                    new ConfigEvent(n_pwmOutput["board"].as<uint8_t>(), (uint8_t)CONFIG_TOPIC_PWM, index++,
                                    (uint8_t)CONFIG_TOPIC_FAST_SWITCH, n_pwmOutput["fastFlipSwitch"].as<uint32_t>()));
        ...
        if (ledStripes) {
            for (YAML::Node n_ledStripe : ledStripes) {
                index = 0;
                m_pRS485Comm->SendConfigEvent(
                    new ConfigEvent(n_ledStripe["board"].as<uint8_t>(), (uint8_t)CONFIG_TOPIC_LED_STRING, index++,
                                    (uint8_t)CONFIG_TOPIC_PORT, n_ledStripe["port"].as<uint32_t>()));
                m_pRS485Comm->SendConfigEvent(
                    new ConfigEvent(n_ledStripe["board"].as<uint8_t>(), (uint8_t)CONFIG_TOPIC_LED_STRING, index++,
                                    (uint8_t)CONFIG_TOPIC_AMOUNT_LEDS, n_ledStripe["amount"].as<uint32_t>()));
                m_pRS485Comm->SendConfigEvent(
                    new ConfigEvent(n_ledStripe["board"].as<uint8_t>(), (uint8_t)CONFIG_TOPIC_LED_STRING, index++,
                                    (uint8_t)CONFIG_TOPIC_LIGHT_UP, n_ledStripe["lightUp"].as<uint32_t>()));
                m_pRS485Comm->SendConfigEvent(
                    new ConfigEvent(n_ledStripe["board"].as<uint8_t>(), (uint8_t)CONFIG_TOPIC_LED_STRING, index++,
                                    (uint8_t)CONFIG_TOPIC_AFTER_GLOW, n_ledStripe["afterGlow"].as<uint32_t>()));

                SendLedConfigBlock(n_ledStripe["lamps"], LED_TYPE_LAMP, n_ledStripe["board"].as<uint8_t>(),
                                   n_ledStripe["port"].as<uint32_t>());
                SendLedConfigBlock(n_ledStripe["flashers"], LED_TYPE_FLASHER, n_ledStripe["board"].as<uint8_t>(),
                                   n_ledStripe["port"].as<uint32_t>());
                SendLedConfigBlock(n_ledStripe["gi"], LED_TYPE_GI, n_ledStripe["board"].as<uint8_t>(),
                                   n_ledStripe["port"].as<uint32_t>());
            }
        }
...
void
PPUC::SetSolenoidState(int number, int state) {
...
void
PPUC::SetLampState(int number, int state) {
