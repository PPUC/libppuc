...
void
RS485Comm::SetLogMessageCallback(PPUC_LogMessageCallback callback, const void* userData) {
    m_logMessageCallback = callback;
    m_logMessageUserData = userData;
}

void
RS485Comm::LogMessage(const char* format, ...) {
...
void
RS485Comm::QueueEvent(Event* event) {
    m_eventQueueMutex.lock();
    m_events.push(event);
    m_eventQueueMutex.unlock();
}
...
bool
RS485Comm::Connect(const char* pDevice) {
...
void
RS485Comm::RegisterSwitchBoard(uint8_t number) {
...
bool
RS485Comm::SendConfigEvent(ConfigEvent* event) {
        ...
        if (sp_blocking_write(m_pSerialPort, m_cmsg, 12, RS485_COMM_SERIAL_WRITE_TIMEOUT)) {
            if (m_debug) {
                ...
                printf("Sent ConfigEvent %02X %d %d %d %d %d %02x%02x%02x%02x %02X %02X\n", m_cmsg[0], m_cmsg[1],
                       m_cmsg[2], m_cmsg[3], m_cmsg[4], m_cmsg[5], m_cmsg[6], m_cmsg[7], m_cmsg[8], m_cmsg[9],
                       m_cmsg[10], m_cmsg[11]);
            }
            return true;
        } else {
            if (m_debug) {
                ...
                printf("Error when sending ConfigEvent %02X %d %d %d %d %d %02x%02x%02x%02x %02X %02X\n", m_cmsg[0],
                       m_cmsg[1], m_cmsg[2], m_cmsg[3], m_cmsg[4], m_cmsg[5], m_cmsg[6], m_cmsg[7], m_cmsg[8],
                       m_cmsg[9], m_cmsg[10], m_cmsg[11]);
            }
        }
    }
...
bool
RS485Comm::SendEvent(Event* event) {
    if (m_pSerialPort != NULL) {
        m_msg[0] = (uint8_t)255;
...
RS485Comm::receiveEvent() {
    if (m_pSerialPort != NULL) {
        ...
        while ((std::chrono::duration_cast<std::chrono::microseconds>(std::chrono::steady_clock::now() - start)).count()
               < 8000) {
...
void
RS485Comm::PollEvents(int board) {
    if (m_debug) {
        // @todo use logger
    ...
    if (SendEvent(event)) {
        ...
        while (!null_event && (event_recv = receiveEvent())) {
            switch (event_recv->sourceId) {
                ...
                case EVENT_NULL: null_event = true; break;
