...
if [ -f "${PROJECT_SOURCE_ROOT}/third-party/include/libserialport.h" ] &&
   [ -f "${PROJECT_SOURCE_ROOT}/third-party/build-libs/linux/x64/libserialport.a" ] &&
   [ -f "${PROJECT_SOURCE_ROOT}/third-party/runtime-libs/linux/x64/libserialport.so.0" ]; then
...
if [ "${LIBSERIALPORT_EXPECTED_SHA}" != "${LIBSERIALPORT_FOUND_SHA}" ] || [ "${LIBSERIALPORT_ARTIFACTS_OK}" -ne 1 ]; then
   ...
   cp .libs/libserialport.so.0 ${PROJECT_SOURCE_ROOT}/third-party/runtime-libs/linux/x64/
   cd ..
...
if [ -d "${PROJECT_SOURCE_ROOT}/third-party/include/yaml-cpp" ] &&
   ls "${PROJECT_SOURCE_ROOT}"/third-party/runtime-libs/linux/x64/libyaml-cpp.so.* >/dev/null 2>&1; then
...
if [ "${YAML_CPP_EXPECTED_SHA}" != "${YAML_CPP_FOUND_SHA}" ] || [ "${YAML_CPP_ARTIFACTS_OK}" -ne 1 ]; then
   ...
   cp -P build/libyaml-cpp.so.* ${PROJECT_SOURCE_ROOT}/third-party/runtime-libs/linux/x64/
   cd ..
