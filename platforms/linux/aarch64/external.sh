...
./configure
make -j${NUM_PROCS}
cp .libs/libserialport.a ${PPUC_SOURCE_ROOT}/third-party/build-libs/linux/aarch64/
cp .libs/libserialport.so.0 ${PPUC_SOURCE_ROOT}/third-party/runtime-libs/linux/aarch64/
cd ..
...
cp -P build/libyaml-cpp.so.* ${PPUC_SOURCE_ROOT}/third-party/runtime-libs/linux/aarch64/
cd ..
